...
export function socketErrorHandling(error: Error, socket: any) {
  if (error instanceof HttpException) {
    if (error.code === HttpStatus.INTERNAL_SERVER_ERROR) {
      Logger.error(error.message);
    }
    return socket.emit("Error", { message: error.message, code: error.code, errors: error.error });
  }
  Logger.error(error.message);
  return socket.emit("Error", { message: error.message, code: 500 });
}
